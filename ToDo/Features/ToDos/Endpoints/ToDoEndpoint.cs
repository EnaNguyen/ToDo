            ...
            {        
                ...
                ToDoProcess.MapPost("/", async (IToDoServices _services, [FromBody] ToDoCreateDTO toDoCreateDTO) =>
                {
                    try
                    {
                        ...
                        return Results.CreatedAtRoute("GetToDo", new { username = create.FullName }, create);
                    }
                    catch (Exception ex)
                    {
                        return Results.Problem(ex.Message);
                ...
                ToDoProcess.MapPut("/", async (IToDoServices _services, [FromBody] ToDoUpdateDTO toDoUpdateDTO, IMapper _mapper) =>
                {
                    try
                    {
                        var update = await _services.UpdateToDoAsync(toDoUpdateDTO);
